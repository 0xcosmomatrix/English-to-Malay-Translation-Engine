...
os.environ.setdefault("OPENROUTER_API_KEY","unset")   # pipeline reads the key only inside call()
from pipeline import safe_name
def one(f,out,config):
    name=safe_name(pathlib.Path(f).stem)
...
def main():
    ...
    reports=[]; missing=[]
    for f in files:
        p=pathlib.Path(a.out)/f"{safe_name(pathlib.Path(f).stem)}-report.json"
    ...
    import json as _j
    tmp=str(pathlib.Path(a.out)/"book-report.json")+".tmp"
    open(tmp,"w").write(_j.dumps(summary,indent=1)); os.replace(tmp,str(pathlib.Path(a.out)/"book-report.json"))
