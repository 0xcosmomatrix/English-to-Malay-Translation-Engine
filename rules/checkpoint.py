...
"""
import json, os, re, sys, collections, pathlib, tempfile, subprocess
...
Return STRICT JSON: {{"candidates":[...]}}"""
    import pipeline as PL   # shared client: retry, telemetry, auth fast-fail
    raw = PL.call(MODEL, prompt, temp=0.0, stage="checkpoint", timeout=240)
    parsed = msml.extract_json(raw) or {}
    cands = [c for c in parsed.get("candidates", [])
             if isinstance(c, dict) and c.get("orig") and c.get("sug")]
    if not cands:
        print("checkpoint: no usable candidates in model reply")
